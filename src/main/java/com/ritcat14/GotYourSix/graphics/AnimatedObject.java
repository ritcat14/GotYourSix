package com.ritcat14.GotYourSix.graphics;

public class AnimatedObject extends Sprite{
   
   //walking animations
   public static AnimatedObject fireKingUp = new AnimatedObject(SpriteSheet.fireKing_up, 32, 32, 2);
   public static AnimatedObject fireKingDown = new AnimatedObject(SpriteSheet.fireKing_down, 32, 32, 2);
   public static AnimatedObject fireKingLeft = new AnimatedObject(SpriteSheet.fireKing_left, 32, 32, 2);
   public static AnimatedObject fireKingRight = new AnimatedObject(SpriteSheet.fireKing_right, 32, 32, 2);
  
   public static AnimatedObject fireSpriteUp = new AnimatedObject(SpriteSheet.fireSprite_up, 32, 32, 2);
   public static AnimatedObject fireSpriteDown = new AnimatedObject(SpriteSheet.fireSprite_down, 32, 32, 2);
   public static AnimatedObject fireSpriteLeft = new AnimatedObject(SpriteSheet.fireSprite_left, 32, 32, 2);
   public static AnimatedObject fireSpriteRight = new AnimatedObject(SpriteSheet.fireSprite_right, 32, 32, 2);
  
   public static AnimatedObject iceKingUp = new AnimatedObject(SpriteSheet.iceKing_up, 32, 32, 2);
   public static AnimatedObject iceKingDown = new AnimatedObject(SpriteSheet.iceKing_down, 32, 32, 2);
   public static AnimatedObject iceKingLeft = new AnimatedObject(SpriteSheet.iceKing_left, 32, 32, 2);
   public static AnimatedObject iceKingRight = new AnimatedObject(SpriteSheet.fireKing_right, 32, 32, 2);
  
   public static AnimatedObject iceSpriteUp = new AnimatedObject(SpriteSheet.iceSprite_up, 32, 32, 2);
   public static AnimatedObject iceSpriteDown = new AnimatedObject(SpriteSheet.iceSprite_down, 32, 32, 2);
   public static AnimatedObject iceSpriteLeft = new AnimatedObject(SpriteSheet.iceSprite_left, 32, 32, 2);
   public static AnimatedObject iceSpriteRight = new AnimatedObject(SpriteSheet.iceSprite_right, 32, 32, 2);
  
   //swimming animations
   public static AnimatedObject fireKingUpSwim = new AnimatedObject(SpriteSheet.fireKing_up, 32, 16, 2);
   public static AnimatedObject fireKingDownSwim = new AnimatedObject(SpriteSheet.fireKing_down, 32, 16, 2);
   public static AnimatedObject fireKingLeftSwim = new AnimatedObject(SpriteSheet.fireKing_left, 32, 16, 2);
   public static AnimatedObject fireKingRightSwim = new AnimatedObject(SpriteSheet.fireKing_right, 32, 16, 2);
  
   public static AnimatedObject fireSpriteUpSwim = new AnimatedObject(SpriteSheet.fireSprite_up, 32, 16, 2);
   public static AnimatedObject fireSpriteDownSwim = new AnimatedObject(SpriteSheet.fireSprite_down, 32, 16, 2);
   public static AnimatedObject fireSpriteLeftSwim = new AnimatedObject(SpriteSheet.fireSprite_left, 32, 16, 2);
   public static AnimatedObject fireSpriteRightSwim = new AnimatedObject(SpriteSheet.fireSprite_right, 32, 16, 2);
  
   public static AnimatedObject iceKingUpSwim = new AnimatedObject(SpriteSheet.iceKing_up, 32, 16, 2);
   public static AnimatedObject iceKingDownSwim = new AnimatedObject(SpriteSheet.iceKing_down, 32, 16, 2);
   public static AnimatedObject iceKingLeftSwim = new AnimatedObject(SpriteSheet.iceKing_left, 32, 16, 2);
   public static AnimatedObject iceKingRightSwim = new AnimatedObject(SpriteSheet.fireKing_right, 32, 16, 2);
  
   public static AnimatedObject iceSpriteUpSwim = new AnimatedObject(SpriteSheet.iceSprite_up, 32, 16, 2);
   public static AnimatedObject iceSpriteDownSwim = new AnimatedObject(SpriteSheet.iceSprite_down, 32, 16, 2);
   public static AnimatedObject iceSpriteLeftSwim = new AnimatedObject(SpriteSheet.iceSprite_left, 32, 16, 2);
   public static AnimatedObject iceSpriteRightSwim = new AnimatedObject(SpriteSheet.iceSprite_right, 32, 16, 2);
  
   //shooting animations
   public static AnimatedObject fireKingUpShoot = new AnimatedObject(SpriteSheet.fireKingShoot_up, 32, 32, 2);
   public static AnimatedObject fireKingDownShoot = new AnimatedObject(SpriteSheet.fireKingShoot_down, 32, 32, 2);
   public static AnimatedObject fireKingLeftShoot = new AnimatedObject(SpriteSheet.fireKingShoot_left, 32, 32, 2);
   public static AnimatedObject fireKingRightShoot = new AnimatedObject(SpriteSheet.fireKingShoot_right, 32, 32, 2);
  
   public static AnimatedObject fireSpriteUpShoot = new AnimatedObject(SpriteSheet.fireSpriteShoot_up, 32, 32, 2);
   public static AnimatedObject fireSpriteDownShoot = new AnimatedObject(SpriteSheet.fireSpriteShoot_down, 32, 32, 2);
   public static AnimatedObject fireSpriteLeftShoot = new AnimatedObject(SpriteSheet.fireSpriteShoot_left, 32, 32, 2);
   public static AnimatedObject fireSpriteRightShoot = new AnimatedObject(SpriteSheet.fireSpriteShoot_right, 32, 32, 2);
  
   public static AnimatedObject iceKingUpShoot = new AnimatedObject(SpriteSheet.iceKingShoot_up, 32, 32, 2);
   public static AnimatedObject iceKingDownShoot = new AnimatedObject(SpriteSheet.iceKingShoot_down, 32, 32, 2);
   public static AnimatedObject iceKingLeftShoot = new AnimatedObject(SpriteSheet.iceKingShoot_left, 32, 32, 2);
   public static AnimatedObject iceKingRightShoot = new AnimatedObject(SpriteSheet.fireKingShoot_right, 32, 32, 2);
  
   public static AnimatedObject iceSpriteUpShoot = new AnimatedObject(SpriteSheet.iceSpriteShoot_up, 32, 32, 2);
   public static AnimatedObject iceSpriteDownShoot = new AnimatedObject(SpriteSheet.iceSpriteShoot_down, 32, 32, 2);
   public static AnimatedObject iceSpriteLeftShoot = new AnimatedObject(SpriteSheet.iceSpriteShoot_left, 32, 32, 2);
   public static AnimatedObject iceSpriteRightShoot = new AnimatedObject(SpriteSheet.iceSpriteShoot_right, 32, 32, 2);
	
	private int frame = 0;
	private Sprite sprite;
	private int rate = 5;
	private int time = 0;
	private int length = -1;
	
	public AnimatedObject(SpriteSheet sheet, int width, int height, int length){
		super(sheet, width, height);
		this.length = length;
		sprite = sheet.getSprites()[0];
		if(length > sheet.getSprites().length) System.out.println("ERROR");
	}
	
	public void update(){
		time++;
		if(time % rate == 0){
			if(frame >= length - 1) frame = 0;
			else frame++;
			sprite = sheet.getSprites()[frame];
		}
	}
	
	public Sprite getSprite(){
		return sprite;
	}
	
	public void setFrameRate(int frames){
		this.frame = frames;
	}
	
	public void setFrame(int index){
		if (index > sheet.getSprites().length -1) {
			System.err.println("Index out of bounds in " + this);
			return;
		}
		sprite = sheet.getSprites()[index];
	}

}
