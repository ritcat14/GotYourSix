...
public class Weapons extends UIPanel {
    ...
    public Weapons() {
        super(new Vector2i((Game.getWindowWidth() - (40 * 6)), Game.getWindowHeight() - 40), new Vector2i(6 * 40, 40),
              ImageUtil.getImage("/ui/weaponBarBack.png"));
        this.x = (Game.getWindowWidth() - (40 * 6));
        this.y = Game.getWindowHeight() - 40;
        ui = new UIManager();
        ui.addPanel(this);
        for (int i = 0; i < 6; i++) {
            int num = (i + 1);
            ...
            //weaponImg = Weapons.resizeImage(weaponImg, weaponImg.getWidth()/2, weaponImg.getHeight()/2);
            weapon =
                     new UIPanel(new Vector2i(x + (i * 40), y), new Vector2i(weaponImg.getWidth() / 2, weaponImg.getHeight() / 2),
                                 weaponImg);
            addComponent(weapon);
            if (num == 1)
                amount = new UILabel(weapon.position, "∞");
            else if (num == 2)
                amount = new UILabel(weapon.position, "" + cannons.size());
            if (StartScreen.state == StartScreen.playerViewState.MF || StartScreen.state == StartScreen.playerViewState.FF) {
                if (num == 3)
                    amount = new UILabel(weapon.position, "" + firearrows.size());
                else if (num == 4)
                    amount = new UILabel(weapon.position, "" + firecannons.size());
                else if (num == 5)
                    amount = new UILabel(weapon.position, "" + fireballs.size());
                else if (num == 6)
                    amount = new UILabel(weapon.position, "" + firewalls.size());
            } else if (StartScreen.state == StartScreen.playerViewState.MI || StartScreen.state == StartScreen.playerViewState.FI) {
                if (num == 3)
                    amount = new UILabel(weapon.position, "" + icearrows.size());
                else if (num == 4)
                    amount = new UILabel(weapon.position, "" + icecannons.size());
                else if (num == 5)
                    amount = new UILabel(weapon.position, "" + iceballs.size());
                else if (num == 6)
                    amount = new UILabel(weapon.position, "" + icewalls.size());
            }
            labels.add(amount);
    ...
    public void changeWeapon(int index) {
        if (index == 1)
            ...
            Projectile.weapon = Projectile.Weapon.CANNON;
        if (StartScreen.state == StartScreen.playerViewState.MF || StartScreen.state == StartScreen.playerViewState.FF) {
            ...
            if (index == 3)
                Projectile.weapon = Projectile.Weapon.FIREDARROW;
            if (index == 4)
                Projectile.weapon = Projectile.Weapon.FIREDCANNON;
            if (index == 5)
                Projectile.weapon = Projectile.Weapon.FIREBALL;
            if (index == 6)
            ...
            if (index == 3)
                Projectile.weapon = Projectile.Weapon.ICEDARROW;
            if (index == 4)
                Projectile.weapon = Projectile.Weapon.ICEDCANNON;
            if (index == 5)
                Projectile.weapon = Projectile.Weapon.ICEBALL;
            if (index == 6)
                Projectile.weapon = Projectile.Weapon.ICEWALL;
        }
        selectedWep = index;
    }
    ...
    public int getAmount() {
        ...
        if (StartScreen.state == StartScreen.playerViewState.MF || StartScreen.state == StartScreen.playerViewState.FF) {
            switch (selectedWep) {
                case 3:
                    return firearrows.size();
                case 4:
                    return firecannons.size();
                case 5:
            ...
            switch (selectedWep) {
                case 3:
                    return icearrows.size();
                case 4:
                    return icecannons.size();
                case 5:
                    return iceballs.size();
                case 6:
                    return icewalls.size();
            }
        }
        return 1;
    }
    ...
    public void removeWep() {
        try {
            ...
            if (StartScreen.state == StartScreen.playerViewState.MF || StartScreen.state == StartScreen.playerViewState.FF) {
                if (selectedWep == 3)
                    firearrows.remove(firearrows.size() - 1);
                if (selectedWep == 4)
                    firecannons.remove(firecannons.size() - 1);
                if (selectedWep == 5)
                    fireballs.remove(fireballs.size() - 1);
                if (selectedWep == 6)
                    firewalls.remove(firewalls.size() - 1);
            } else if (StartScreen.state == StartScreen.playerViewState.MI || StartScreen.state == StartScreen.playerViewState.FI) {
                if (selectedWep == 3)
                    icearrows.remove(icearrows.size() - 1);
                if (selectedWep == 4)
                    icecannons.remove(icecannons.size() - 1);
                if (selectedWep == 5)
                    iceballs.remove(iceballs.size() - 1);
                if (selectedWep == 6)
                    icewalls.remove(icewalls.size() - 1);
            }
        } catch (Exception e) {
    ...
    private void updateLabels() {
        for (int i = 0; i < labels.size(); i++) {
            if (i == 0)
                continue;
            if (i == 1)
                labels.get(i).setText("" + cannons.size());
            if (StartScreen.state == StartScreen.playerViewState.MF || StartScreen.state == StartScreen.playerViewState.FF) {
                if (i == 2)
                    labels.get(i).setText("" + firearrows.size());
                if (i == 3)
                    labels.get(i).setText("" + firecannons.size());
                if (i == 4)
                    labels.get(i).setText("" + fireballs.size());
                if (i == 5)
                    labels.get(i).setText("" + firewalls.size());
            } else if (StartScreen.state == StartScreen.playerViewState.MI || StartScreen.state == StartScreen.playerViewState.FI) {
                if (i == 2)
                    labels.get(i).setText("" + icearrows.size());
                if (i == 3)
                    labels.get(i).setText("" + icecannons.size());
                if (i == 4)
                    labels.get(i).setText("" + iceballs.size());
                if (i == 5)
                    labels.get(i).setText("" + icewalls.size());
            }
        }
    }
